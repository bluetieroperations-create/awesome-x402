...
- [Blackwall](https://x402.blackwalltier.com/) - Pre-signature payment-risk oracle for x402: returns GO / HOLD / STOP before an agent signs a payment, from counterparty reputation, price-anomaly (quoted amount vs the counterparty's own per-payer median, wash-trade-hardened), and OFAC sanctioned-address screening — folded into one verdict with a signed receipt. Verdict only, never custody. Live on Base. Also an MCP server (`forecast_payment`). ([compare](https://x402.blackwalltier.com/payment-risk-oracles-compared)) ([/.well-known/x402](https://agent-egress-proxy.onrender.com/.well-known/x402))
